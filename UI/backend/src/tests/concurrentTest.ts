...
async function testSimpleConcurrent(): Promise<TestResult> {
  ...
  const numUsers = 20;
  const expectedAllowed = getNumberEnv('QUEUE_MAX_CONCURRENT_USERS', 5);
...
async function testSequentialWaves(): Promise<TestResult> {
  ...
  const maxConcurrentUsers = getNumberEnv('QUEUE_MAX_CONCURRENT_USERS', 5);
  const success = totalAllowed <= maxConcurrentUsers * numWaves;
  console.log(`\n📊 Total Results:`);
  console.log(`   Total Allowed: ${totalAllowed}`);
  console.log(`   Total Queued: ${totalQueued}`);
  console.log(`   Status: ${success ? '✅ PASSED' : '❌ FAILED'}`);

  return { total: waveSize * numWaves, allowed: totalAllowed, queued: totalQueued, expectedAllowed: maxConcurrentUsers, success };
}
