...
export default function App() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // --- GIẢ LẬP HÀNG ĐỢI CỦA SERVER KHI CÓ NHIỀU CONCURRENT USERS ---
      // Nếu comment phần này ra, ở backend thực tế, bạn sẽ dùng Server-Sent Events (SSE) 
      // hoặc WebSockets để backend báo về trạng thái "bạn đang ở vị trí số mấy".
      let currentPosition = Math.floor(Math.random() * 4) + 1; // Random vị trí từ 1-4
      setQueuePosition(currentPosition);

      // Cho vị trí tụt dần về 0 để giả lập xếp hàng chờ
      while (currentPosition > 0) {
        await new Promise(resolve => setTimeout(resolve, 1500)); // Đợi 1.5s mỗi lượt
        currentPosition--;
        if (currentPosition > 0) setQueuePosition(currentPosition);
      }
      // Hết hàng đợi
      setQueuePosition(null);

      // --- KẾT THÚC HÀNG ĐỢI, BẮT ĐẦU XỬ LÝ RESPONSE ---
      // TODO: Gắn API backend vào đây (chỉ gọi API sau khi đến lượt hoặc API đã xử lý xong queue)

      // Sử dụng kho dữ liệu nội bộ
      const mockResponses = knowledgeBase.length > 0 ? knowledgeBase : ["Xin lỗi, tôi chưa có dữ liệu nào để trả lời."];
            ...
            {messages.length === 0 && (
              ...
              >
                <h1 className="font-headline text-5xl font-extrabold tracking-tight mb-6 text-transparent bg-clip-text bg-gradient-to-r from-primary via-tertiary to-primary leading-tight">
                  How can I assist your <br/>creative process today?
                </h1>
                <p className="text-on-surface-variant text-lg leading-relaxed font-medium mb-12">
                  Harness the power of Ethereal Interface to draft documents, analyze complex data, or explore new dimensions of your imagination.
                </p>

                {/* Bento Prompt Suggestions */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                  {[
                    { icon: Lightbulb, color: 'text-primary', title: 'Ideate', desc: 'Brainstorm 5 unique startup ideas for sustainable urban living.' },
                    { icon: Code2, color: 'text-tertiary', title: 'Analyze', desc: 'Explain the implementation of a recursive neural network in Python.' },
                    { icon: FileEdit, color: 'text-secondary', title: 'Refine', desc: 'Rewrite this email to sound more empathetic and professional.' }
                  ].map((item, i) => (
                    <motion.div 
                      key={i}
                      initial={{ opacity: 0, y: 20 }}
          ...
          <AnimatePresence>
            {queuePosition !== null && (
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }} 
                exit={{ opacity: 0, y: 10 }} 
                className="mb-4 bg-surface-container-high border border-primary/20 px-6 py-2 rounded-full text-sm font-medium text-primary shadow-lg flex items-center gap-3 pointer-events-auto"
              >
                <Loader2 className="w-4 h-4 animate-spin" />
                Server đang tải. Bạn đang ở vị trí số {queuePosition} trong hàng đợi...
              </motion.div>
            )}
          </AnimatePresence>
